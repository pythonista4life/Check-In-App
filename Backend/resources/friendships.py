...
from schemas.schemas import SearchFriendResponseSchema, AddFriendResponseSchema, GetFriendsResponseSchema, DeleteFriendResponseSchema, AcceptRejectFriendResponseSchema, GetSentPendingFriendsResponseSchema, GetReceivedPendingFriendsResponseSchema
...
class AddFriend(MethodView):
    ...
    def post(self, friend_username):
        ...
        if existing_friendship:
            abort(400, message="Friend request already sent or user is already a friend.")
...
class GetFriends(MethodView):
    ...
    def get(self):
        ...
        friendships = FriendshipModel.query.filter_by(user_id=current_user_id, status="accepted").all()
        friends = [{"id": f.friend_id, "username": f.friend.username} for f in friendships]
...
class GetSentPendingFriends(MethodView):
    ...
    def get(self):
        ...
        pending_friendships = FriendshipModel.query.filter_by(user_id=current_user_id, status="pending").all()
        pending_friends = [{"id": f.friend_id, "username": f.friend.username} for f in pending_friendships]
...
class GetReceivedPendingFriends(MethodView):
    ...
    def get(self):
        ...
        received_requests = FriendshipModel.query.filter_by(friend_id=current_user_id, status="pending").all()
        received_friends = [{"id": f.user_id, "username": f.user.username} for f in received_requests]
...
class DeleteFriend(MethodView):
    ...
    def delete(self, friend_username):
        ...
        friendship = FriendshipModel.query.filter(
            ((FriendshipModel.user_id == current_user_id) & (FriendshipModel.friend_id == friend.id)) |
            ((FriendshipModel.user_id == friend.id) & (FriendshipModel.friend_id == current_user_id))
        ).first()
