...
# Custom Questions Arguments Schema
class CustomQuestionSchema(Schema):
    question = fields.Str(required=True)


# Custom Questions Response Schema
...
# Delete Custom Questions Response Schema
...
class RegisterArgsSchema(Schema):
    ...
    confirm_email = fields.Email(
        required=True, description="Confirmation of the user's email address"
    )
    ...
    def check_email_match(self, data, **kwargs):
        ...
        if data.get("email") != data.get("confirm_email"):
            raise ValidationError(
                "Email and confirm email must match.", field_name="confirm_email"
            )
        return data
...
class LoginResponseSchema(Schema):
    access_token = fields.String(required=True, description="JWT Token")
    message = fields.String(
        required=True,
...
class PasswordResetRequestSchema(Schema):
    email = fields.Email(
        required=True, description="The registered email address of the user"
    )
...
class ResetCodeRequestSchema(Schema):
    ...
    reset_code = fields.String(
        required=True, description="Verification code for password reset"
    )
...
class GetFriendsResponseSchema(Schema):
    friendships = fields.List(
        fields.Nested(UserSchema), description="A list of friendships."
    )


# A list of pending friend requests sent by the user.
class GetSentPendingFriendsResponseSchema(Schema):
    pending_friends = fields.List(
        fields.Nested(UserSchema),
        description="A list of pending friend requests sent by the user.",
    )


# A list of pending friend requests received by the user.
class GetReceivedPendingFriendsResponseSchema(Schema):
    received_requests = fields.List(
        fields.Nested(UserSchema),
        description="A list of pending friend requests received by the user.",
    )
